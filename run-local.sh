...
# Example: OPENSTACK_CONFIG_DOCKER_IMAGE=config-prebuild .automation/run-local.sh .automation/pipeline/apply.sh -p ansible/um6p-project.yml -- --env OPENSTACK_CONFIG_VAULT_PASSWORD=$(< ~/.kayobe-vault-pass) --env OPENSTACK_CONFIG_OPENRC="$(< ~/will/openrc-um6p-config)"
